...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(flag.CommandLine.Output(), "Example (built dependency graph comparison):\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  git checkout master\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  go list -json -deps -test ./... | jq -s 'map(select(.Module != null) | .Module) | unique | .[]' > go.list.json.master\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  git checkout mybranchwithchanges\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  go list -json -deps -test ./... | jq -s 'map(select(.Module != null) | .Module) | unique | .[]' > go.list.json.mybranchwithchanges\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  golistcmp go.list.json.master go.list.json.mybranchwithchanges \n")
		fmt.Fprintf(flag.CommandLine.Output(), "\n")
		fmt.Fprintf(flag.CommandLine.Output(), "Example (full dependency graph comparison):\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  git checkout master\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  go list -m -json all > go.list.json.master\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  git checkout mybranchwithchanges\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  go list -m -json all > go.list.json.mybranchwithchanges\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  golistcmp go.list.json.master go.list.json.mybranchwithchanges\n")
		fmt.Fprintf(flag.CommandLine.Output(), "\n")
